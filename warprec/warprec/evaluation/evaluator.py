...
class Evaluator:
    ...
    def _compute_metrics_step(self, predictions, eval_batch, user_indices, candidates):
        ...
        for k, metric_instances in self.metrics.items():
            for metric in metric_instances:
                ...
                if f"top_{k}_indices" in precomputed_blocks[k]:
                    update_kwargs["top_k_indices"] = precomputed_blocks[k][f"top_{k}_indices"]
                if f"top_{k}_values" in precomputed_blocks[k]:
                    update_kwargs["top_k_values"] = precomputed_blocks[k][f"top_{k}_values"]
                if f"top_{k}_binary_relevance" in precomputed_blocks[k]:
                    update_kwargs["top_k_binary_relevance"] = precomputed_blocks[k][f"top_{k}_binary_relevance"]
    ...
    def compute_results(self) -> Dict[int, Dict[str, float | Tensor]]:
        ...
        Normalizes metric keys to be cutoff-independent (strips @k suffix),
        so that results[5]['CS'] and results[10]['CS'] both exist.
        ...
        for k, metric_instances in self.metrics.items():
            ...
            for metric in metric_instances:
                ...
                # Normalize keys: strip @k suffix so all cutoffs use the same key
                # e.g. 'CS@10' -> 'CS', 'nDCG@5' -> 'nDCG', 'AUC' -> 'AUC'
                normalized = {
                    re.sub(r'@\d+$', '', mk): mv
                    for mk, mv in metric_result.items()
                }
                results[k].update(normalized)
        return results
    ...
    def print_console(
        ...
        Args:
            results (Dict[int, Dict[str, float | Tensor]]): The dictionary containing
                all the results. Keys are cutoff-independent (no @k suffix).
            header (str): The header of the table to be printed.
            max_metrics_per_row (int): The number of metrics to print in each row.
        """
        # Collect all unique metric keys from the first cutoff (keys are normalized, no @k)
        ...
        for chunk_idx, chunk_keys in enumerate(chunks):
            ...
            for k, metrics in results.items():
                ...
                for key in chunk_keys:
                    # Keys are normalized (no @k suffix), so direct lookup works for all cutoffs
            ...
            # Display headers with @k suffix from the first cutoff for clarity
            # e.g. show "CS@5" as column header (using first cutoff's k value)
            display_headers = [f"{key}@{first_cutoff_key}" if "@" not in key else key
                               for key in chunk_keys]

            table = tabulate(
                _tab,
                headers=["Cutoff"] + display_headers,
                tablefmt="grid",
            )
